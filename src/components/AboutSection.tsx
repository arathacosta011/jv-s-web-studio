  ...
  return (
    <section id="about" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="container relative">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-border/40 shadow-card">
              <img src={jvLaptop} alt="JV — founder of HEEM, working on the brand" loading="lazy" className="w-full aspect-[3/4] object-cover object-top" />
              <div className="absolute inset-0 bg-gradient-to-t from-background/50 via-transparent to-transparent" />
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <p className="text-primary font-body text-[11px] tracking-[0.3em] uppercase mb-3">The Founder</p>
            <h2 className="font-display text-3xl md:text-5xl font-extrabold mb-8">MEET <span className="text-gradient-violet">JV</span></h2>
            <p className="text-muted-foreground font-body leading-[1.8] mb-6 text-[15px]">
              Jose Velasco — known as JV — started cutting hair as a kid, learning the craft before most people pick up their first job. What began in a living room with a wooden chair became one of San Diego's most respected names in barbering.
            </p>
            <p className="text-muted-foreground font-body leading-[1.8] mb-10 text-[15px]">
              Every HEEM product was born in the chair, built for real clients getting real cuts. From a kid with clippers to running his own product line and warehouse — JV turned passion into a brand trusted by barbers worldwide.
            </p>
            ...
            <div className="grid grid-cols-3 gap-8">
              {[
                ...
                return item.href ? (
                  <a key={i} href={item.href} target="_blank" rel="noopener noreferrer" className="hover:opacity-80 transition-opacity">{content}</a>
                ) : (
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="mt-24">
          <p className="text-center text-primary font-body text-[11px] tracking-[0.3em] uppercase mb-10">The Journey</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {journey.map((item, i) => (
              ...
              >
                <img src={item.image} alt={item.caption} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
